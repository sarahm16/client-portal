...
function HvacPmEquipment() {
    ...
    const fetchEquipment = async () => {
      try {
        ...
        const response = await azureClient.post(
          ...
          {
            query: `SELECT * FROM c WHERE c.siteId = '${siteId}' AND c.service = 'HVAC PM'`,
          },
        );
  ...
  const calculateCompletion = (eq) => {
    ...
    const requiredImages = 2; // Before and After images required
    const completedImages = [eq.beforeImage, eq.afterImage].filter(
      (img) => img?.url,
    ).length;

    const checklistCompleted = Object.values(eq.checklist || {}).filter(
      (val) => val,
    ).length;
    const checklistTotal = Object.keys(eq.checklist || {}).length;

    const totalRequired = checklistTotal + requiredImages;
    const totalCompleted = checklistCompleted + completedImages;
            ...
            >
              <CardContent>
                ...
                >
                  ...
                  HVAC PM Equipment Details ({equipment.length} units)
                </Typography>
...
export default HvacPmEquipment;
