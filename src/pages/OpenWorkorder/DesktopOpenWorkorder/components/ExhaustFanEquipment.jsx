...
function ExhaustFanEquipment() {
    ...
    const fetchEquipment = async () => {
      try {
        ...
        const response = await azureClient.post(
          ...
          {
            query: `SELECT * FROM c WHERE c.siteId = '${siteId}' AND c.service = 'Exhaust Fan PM'`,
          },
        );
  ...
  const calculateCompletion = (eq) => {
    ...
    const requiredFields = ["fanType"];

    const beforeImage = eq.beforeImage?.url ? 1 : 0;
    const afterImage = eq.afterImage?.url ? 1 : 0;

    const completedImages = beforeImage + afterImage;
    ...
    const checklistCompletion = Object.values(eq.checklist || {}).filter(
      (item) => item === true,
    ).length;

    const totalRequired = requiredFields.length + requiredImages + 2; // +2 for checklist items
    const totalCompleted =
      completedFields + completedImages + checklistCompletion;
                                ...
                                <ImageList cols={2} gap={8}>
                                  {images.map((image, imgIndex) => (
                                    <ImageListItem
                                      key={imgIndex}
                                      onClick={() =>
                                        openImageModal(images, imgIndex)
                                      }
                                      sx={{ cursor: "pointer" }}
                                    >
                                      <img
...
export default ExhaustFanEquipment;
